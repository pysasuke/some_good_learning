...
//        boolean b4 = 'A' instanceof Character;    //编译不通过：instanceof只能用于对象的判断，不能用于基本类型的判断
        //空对象是否是String的实例
        boolean b5 = null instanceof String;    //false  instanceof特有规则：若左边操作数是null，结果就直接返回false，不再运算右边操作数是什么类
        //类型转换后的空对象是否是String的实例
        boolean b6 = (String) null instanceof String;    //false    null是一个万用类型，也可以说它没有类型，即使做类型转换还是个null
        //Date对象是否是String的实例
//        boolean b7 = new Date() instanceof String;    //编译不通过    instanceof操作符的左右操作数必须有继承或者实现关系，否则编译会失败
